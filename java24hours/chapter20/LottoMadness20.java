...
public class LottoMadness20 extends JFrame {
    LottoEvent20 lotto = new LottoEvent20(this);
    ...
    public LottoMadness20() {
        ...
        ActionListener al = (listener) -> {
            String command = listener.getActionCommand();
            ...
            if (command.equals("Reset")) {
                lotto.clearAllFields();
            }
        };
    
        ItemListener il = (listener) -> {
            Object item = listener.getItem();
            if (item == quickpick) {
                for (int i = 0; i < 6; i++) {
                    int pick;
                    do {
                        pick = (int) Math.floor(Math.random() * 50 + 1);
                    } while (lotto.numberGone(pick, numbers, i));
        ...
        // Add listeners
        quickpick.addItemListener(il);
        personal.addItemListener(il);
