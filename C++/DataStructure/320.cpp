...
Node *createBiTreeByLTAndLVR(const vector<int> &LT, iter LVR_begin, iter LVR_end) { //在分析时，将整个二叉树分解成只有根结点和左右孩子的最小子树
    static auto it = LT.begin();
    Node *root{};
    if (LVR_begin < LVR_end) {
        root = new Node(*LT_begin); //层序的首结点为根结点
        auto LVR_root = find(LVR_begin, LVR_end, root->m_val); //在中序中定位根结点
        root->lchild = createBiTreeByLTAndLVR(LT_begin + 1, LT_end, LVR_begin, LVR_root);
        root->rchild = createBiTreeByLTAndLVR(LT_begin + 2, LT_end, LVR_root + 1, LVR_end);
    }
    return root;
}
