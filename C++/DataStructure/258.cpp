...
int main() {
    ...
    string next = begin;
    while (next != "-1") {
        auto i = find_if(NodeVec.begin(), NodeVec.end(),
                         [next](const Node &node) -> bool { return node.m_address == next; });
    ...
    for (Node node: sortedVec) {
        if (find_if(uniqueVec.begin(), uniqueVec.end(),
                    [node](Node uni) -> bool { return abs(node.m_val) == abs(uni.m_val); }) ==
            uniqueVec.end()) { //如果没出现过
